# Docker Image for CHT Coupler

This repository contains Docker images for Conjugate Heat Transfer,
based on OVERFLOW, OVERTURE or OpenFOAM for fluids, and CalculiX 
or FEniCS for solids. The Docker images are based on Ubuntu 17.10.
It also supports a lightweight LXDE Windows Manager and Jupyter
Notebook, which can be displayed in your web browser. You can use 
this Docker image on 64-bit Linux, Mac or Windows.
...
1. Docker only supports 64-bit Windows 10 Pro or higher. If you have Windows 8 or Windows 10 Home, you need to upgrade your Windows operating system before installing Docker.
...
[`cht_jupyter.py`](https://raw.githubusercontent.com/unifem/cht-coupler/master/cht_jupyter.py)
...
```
curl https://raw.githubusercontent.com/unifem/cht-coupler/master/cht_jupyer.py -outfile cht_jupyer.py
```
On Linux or Mac, start a terminal, use the `cd` command to change to the working directory, and then run the following command:
```
curl -s -O https://raw.githubusercontent.com/unifem/cht-coupler/master/cht_jupyer.py
```

After downloading the script, you can start the Docker image using the command
```
python cht_jupyer.py -p
```
...
```
python cht_jupyter.py -h
```

### Running the Docker Image with Desktop Environment
Besides using the Docker Image as a Jupyter Notebook server, you can also start it in an X-Windows desktop environment. Simply replace `cht_jupyter.py` with `cht_desktop.py` in the preceding commands. That is, on Windows use the following commands in the PowerShell:
```
curl https://raw.githubusercontent.com/unifem/cht-coupler/master/cht_desktop.py -outfile cht_desktop.py
python cht_desktop.py -p
```
and on Linux and Mac use the following commands in a terminal:
```
curl -s -O https://raw.githubusercontent.com/unifem/cht-coupler/master/cht_desktop.py
python cht_desktop.py -p
```
...
```
python cht_jupyter.py
```
or
```
python cht_desktop.py
```
